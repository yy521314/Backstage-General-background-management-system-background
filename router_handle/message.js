...
exports.editMessage = (req, res) => {
    ...
    const returnOldDepartment = (id) => {
        return new Promise((resolve) => {
            const sql =
                "select message_receipt_object from message where id = ?";
...
exports.firstDelete = (req, res) => {
    ...
    db.query(
        sql, [message_status, message_delete_time, req.body.id],
        (err, result) => {
            if (err) return res.cc(err);
            res.send({
                status: 0,
...
exports.recover = (req, res) => {
    ...
    db.query(
        sql, [message_status, message_update_time, req.body.id],
        (err, result) => {
            if (err) return res.cc(err);
            res.send({
                status: 0,
